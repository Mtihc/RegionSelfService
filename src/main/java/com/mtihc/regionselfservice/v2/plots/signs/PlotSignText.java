...
public abstract class PlotSignText<T extends IPlotSignData> {
	 ...
	 * @param lines The lines of text on a wooden sign
	...
	/**
	 * Create the object that is a subclass of <code>PlotSignText</code>. 
	 * Could be <code>ForSaleSignText</code> or <code>ForRentSignText</code>.
	 * @param plotWorld The world data
	 * @param lines The lines of text on the wooden sign
	...
	/**
	 * Constructor.
	 * @param plotWorld The data of the world
	 ...
	 * @throws SignException thrown when the lines on the sign are invalid
	 */
	public PlotSignText(PlotWorld plotWorld, String[] lines) throws SignException {
		...
		if(plot == null || plot.getRegion() == null) {
			throw new SignException("Region '" + regionId + "' does not exist.");
		}
		this.plot = plotWorld.getPlot(regionId);
	}
	...
	public static class ForSaleSignText extends PlotSignText<ForSaleSignData> {
		...
		public void setSellCost(String[] lines, double cost) {
			String costString;
			if(cost % 1 == 0) costString = String.valueOf((int) cost);
			else costString = String.valueOf(cost);
	...
	public static class ForRentSignText extends PlotSignText<ForRentSignData> {
		...
		public static void setRentCost(String[] lines, double cost, long millisec) {
			...
			String costString;
			if(cost % 1 == 0) costString = String.valueOf((int) cost);// remove the .0
			else costString = String.valueOf(cost);
		...
		public ForRentSignText(PlotWorld plotWorld, String[] lines) throws SignException {
			...
			try {
				...
				time = new TimeStringConverter().convert(
						lines[1].split(separator)[1].trim());				
			} catch(Exception e) {
				time = 0;
			}
			if(time <= 0) {
			...
			try {
				...
				try {
					...
					rentCost = Double.parseDouble(costOrPlayer);
